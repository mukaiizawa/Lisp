...
(defvar *debug-tk* nil
  "verbosity of debug messages, if true, then all communication.with tk is echoed to stdout")
...
(defmethod initialize-instance :after ((item treeitem) &key)
  ...
  (format-wish "~a insert ~a end -id ~a -text \"~a\" ~@[-tag ~a~] ~@[-image ~a~]"
               (widget-path (tree item)) (if (master item)
                                           (name (master item))
                                           "{}")
    ...
    (let ((itemtype (pop item))
      ...
      (cond
        ((eq itemtype :rectangle)
         (format stream "~a create rectangle ~a ~a ~a ~a "
                 cpath (number) (number) (number) (number))
         (args))
        ((eq itemtype :polygon)
         (format stream "~a create polygon ~a"
                 cpath (process-coords (pop item)))
         (args)) 
        ((eq itemtype :arc)
         (format stream "~a create arc ~a ~a ~a ~a "
                 cpath (number) (number) (number) (number))
         (args))
        ((eq itemtype :oval)
         (format stream "~a create oval ~a ~a ~a ~a "
                 cpath (number) (number) (number) (number))
         (args))
        ((eq itemtype :line)
         (format stream "~a create line ~a ~a ~a ~a "
                 cpath (number) (number) (number) (number))
         (args))
        ((eq itemtype :image)
...
(defun choose-directory (&key (initialdir (namestring *default-pathname-defaults*))
  ...
  (format-wish "senddatastring [tk_chooseDirectory ~@[ -initialdir \"~a\"~]~@[ -parent ~a ~]~@[ -title {~a}~]~@[ -mustexist ~a~]]"
               (tkescape2 initialdir) (and parent (widget-path parent)) title (and mustexist 1))
  (read-data))
...
(defun message-box (message title type icon &key parent)
  ...
  (format-wish "senddatastring [tk_messageBox -message \"~a\" -title {~a} -type ~(~a~) -icon ~(~a~)~@[ -parent ~a~]]"
               (tkescape2 message) title type icon (and parent (widget-path parent)))
  (read-keyword))
  ...
  (cond
    ...
    (t
      (let* ((name (create-name)))
        ...
        (send-wish (format nil "~A add command -label {~A} -command {puts -nonewline  {(\"~A\")};flush $server}"
...
(defun process-events ()
  "A function to temporarliy yield control to wish so that pending events can be processed,
  useful in long loops or loops that depend on tk input to terminate"
  (let (event)
    (loop 
...
(defmacro with-ltk ((&rest keys &key (debug 2) stream serve-event &allow-other-keys) &body body)
