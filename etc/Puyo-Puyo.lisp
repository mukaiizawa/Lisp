...
(instance-vars
  ...
  frame text-area canvas1 canvas-next1
...
(defmethod put-puyo (canvas (puyo puyo) (point coordinate))
...
(defun put-next-puyos ()
  (setf curr-puyos (loop for i from 0 to 1 collect
                         (let ((puyo (nth i next-puyos)))
                           (itemdelete canvas-next1 (puyo-id puyo))
                           (put-puyo canvas1 puyo (make-vector 2 i))
                           puyo))
...
(defun create-next-puyos ()
  (loop for i from 0 to 1 collect
        (let ((puyo (copy-puyo (nth (random (length puyos) (make-random-state t))
                                    puyos))))
          (put-puyo canvas-next1 puyo (make-vector 0 i))
          puyo)))
