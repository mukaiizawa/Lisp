...
(defmethod parse-quote-block ((ar ahead-reader) &optional (level 0))
  (let ((acc))
    (push (subseq (get-line ar) (1+ level)) acc)
    (while (char= (get-next ar) #\>)
      (when (stringp (car acc)) (push `(:br) acc))
      (let ((next-level (quote-level ar)))
        (cond ((= next-level level)
               (push (subseq (get-line ar) (1+ level)) acc))
              ((< next-level level) (return-from nil))
              (t (push (parse-quote-block ar next-level) acc)))))
    `(:blockquote ,@(nreverse acc))))
...
(defmethod parse-preformatted-text-block ((ar ahead-reader))
  (let (line)
...
(defmethod parse-list-block ((ar ahead-reader) &optional (level 0))
  (let ((acc) (ul? (char= (get-next ar) #\-)))
    (while (list-line? ar)
      (let ((next-level (list-level ar)))
        (cond ((= next-level level)
               (push `(:li ,(subseq (get-line ar) (1+ level))) acc))
              ((< next-level level) (return-from nil))
              (t (push (parse-list-block ar next-level) acc)))))
    `(,(if ul? :ul :ol) ,@(nreverse acc))))
...
(defmethod parse-statement ((ar ahead-reader))
  (cond ((char= (get-next (read-blank ar)) #\#) (parse-header ar))
        ...
        ((preformatted-text? ar) (parse-preformatted-text-block ar))
